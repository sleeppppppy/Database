...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Insert(const KeyType &key, const ValueType &value, const KeyComparator &keyComparator)
    -> int {
  auto array_index = KeyIndex(key, keyComparator);
  if (array_index == GetSize()) {
    *(array_ + array_index) = {key, value};
    IncreaseSize(1);
    return GetSize();
  }

  if (keyComparator(array_[array_index].first, key) == 0) {
    return GetSize();
  }

  std::move_backward(array_ + array_index, array_ + GetSize(), array_ + GetSize() + 1);
  *(array_ + array_index) = {key, value};
...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::Lookup(const KeyType &key, ValueType *value, const KeyComparator &keyComparator) const
    -> bool {
  int array_index = KeyIndex(key, keyComparator);
  if (array_index == GetSize() || keyComparator(array_[array_index].first, key) != 0) {
    return false;
  }
  *value = array_[array_index].second;
  return true;
}
...
auto B_PLUS_TREE_LEAF_PAGE_TYPE::RemoveAndDeleteRecord(const KeyType &key, const KeyComparator &keyComparator) -> int {
  ...
  std::move(array_ + array_index + 1, array_ + GetSize(), array_ + array_index);
  IncreaseSize(-1);
  return GetSize();
}
